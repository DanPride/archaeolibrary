...
confirm_logged_in("MCulture");
	function findAll() {
		...
		if($_POST['MC_Class'] == "All"){
			$where = "WHERE " . 'DFC' . " LIKE " . $Year;
		} else {
		...
		$query_recordset = "SELECT Name,Field,Square,Locus,Basket,Description,Quantity,Saved,Comments,Disposition,DFC,CreateDate FROM `Objects` $where $order";
