...
function reportsHeader($Title = "Archaeolibrary Excavation Management Software",$MCClass = "", $MCYear = "", $Keywords = array("Archaeology","Software","Biblical","ASOR","Analysis","Excavation","Antiquities Authority"),$PageDescrip = "Archaeological Excavation Management and Analysis Software", $Selection = "", $Year = ""){
	...
	$Header .= "<link href=\"stylesheets/public.css\" rel=\"stylesheet\" type=\"text/css\" />";
	$Header .= "</HEAD>";
	...
	$Header .="</td><td VALIGN=TOP ALIGN=CENTER  WIDTH=\"400\"><h2>Tel Gezer<br>$Title<BR>$MCClass<br>$MCYear</h2>";
	$Header .="</td><td VALIGN=BOTTOM align=CENTER WIDTH=\"200\"><br><ul class=\"entry\"><li><a href=\"index.php\">Home</a></li></ul>";
	$Header .="</td></tr></Table>";		
	echo $Header;
}
...
function confirm_logged_in($PageTrack = "Unknown") {
	global $conn;
	if(!logged_in()) {
		redirect_to("GezerLogin.php");
	} else {	
		$Page = $PageTrack;
	/*	$query = sprintf("SELECT Name FROM Pages WHERE Name = '{$Page}'");
		$result = mysql_query($query, $conn);
		$Count = mysql_num_rows($result);
		if($Count == 0)
		{				
			$query_insert = sprintf("INSERT INTO `Pages` (Name,Status) VALUES (%s,%s)" ,		
	...
	$query_insert = sprintf("INSERT INTO `Logs` (Name,Page,IP,Referer,RHost,Host,Agent,Server,Date) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,now())" ,		
