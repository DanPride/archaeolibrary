...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `Vessels` (Type,Vessel) VALUES (%s,%s)" ,			GetSQLValueString($_REQUEST["Type"], "text"), # 
			GetSQLValueString($_REQUEST["Vessel"], "text")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `Vessels` WHERE Id = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `Vessels` SET Type = %s,Vessel = %s WHERE Id = %s", 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `Vessels` WHERE Id = %s", 
			GetSQLValueString($row_recordset["Id"], "int")
		);
