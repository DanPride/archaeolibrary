...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `Periods` (ListOrder,Visible,Period,Code,SubPeriod) VALUES (%s,%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["ListOrder"], "int"), # 
...
function update() {
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `Periods` SET ListOrder = %s,Visible = %s,Period = %s,Code = %s,SubPeriod = %s WHERE Id = %s", 
